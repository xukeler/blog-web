...
Pinyin.GetQP=function(l1){
    let l2 = l1.length;
    let I1 = "";
   // let reg = new RegExp('[a-zA-Z0-9\- ]'); //空格转换成-
    let reg = new RegExp('[a-zA-Z0-9\]');  //去掉空格
    for (let i=0;i<l2;i++) {
        let val = l1.substr(i,1);
        let name = this._arraySearch(val,this._pinyin);
        if(reg.test(val)) {
            I1 += val;
        } else if (name!==false) {
            I1 += name;
...
Pinyin.GetHP = function(str){
  let result = "";
  //let regStr = str.replace(/(^\s*)|(\s*$)/g,'');
  let regStr = str.replace(/\s/g, '');
  let strlh = regStr.length;
  if (strlh == 2) {
      result = Pinyin.GetQP(regStr);
  } else {
      for(let i = 0;i < strlh; i++){
            if(i!=0){
